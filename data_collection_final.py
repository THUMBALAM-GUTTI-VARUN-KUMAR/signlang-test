...
current_dir = oss.path.dirname(oss.path.abspath(__file__))
data_root = oss.path.join(current_dir, "AtoZ_3.1")
oss.makedirs(oss.path.join(data_root, "A"), exist_ok=True)
white_path = oss.path.join(current_dir, "white.jpg")
    ...
    try:
        ...
        if interrupt & 0xFF == ord('n'):
            ...
            if ord(c_dir)==ord('Z')+1:
                c_dir='A'
            flag = False
            oss.makedirs(oss.path.join(data_root, c_dir), exist_ok=True)
            count = len(oss.listdir(oss.path.join(data_root, c_dir)))
        ...
        if flag==True:
            ...
            if suv==180:
                flag=False
            if step%3==0:
                out_dir = oss.path.join(data_root, c_dir)
                oss.makedirs(out_dir, exist_ok=True)
                cv2.imwrite(oss.path.join(out_dir, str(count) + ".jpg"), skeleton1)
