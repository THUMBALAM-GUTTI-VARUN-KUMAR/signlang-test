...
data_root = os.path.join(current_dir, "dataset_collection")
os.makedirs(os.path.join(data_root, "Gray_imgs", "A"), exist_ok=True)
os.makedirs(os.path.join(data_root, "Gray_imgs_with_drawing", "A"), exist_ok=True)
...
p_dir = "A"
c_dir = "a"
count = len(os.listdir(os.path.join(data_root, "Gray_imgs", p_dir)))
...
while True:
    try:
        ...
        if hands:
            ...
            hands, _ = hd.findHands(white, draw=False, flipType=True)
            if hands:
                hand = hands[0]
                x, y, w, h = hand['bbox']
        ...
        if interrupt & 0xFF == ord('n'):
            ...
            if ord(p_dir)==ord('Z')+1:
                ...
                c_dir="a"
            flag = False
            os.makedirs(os.path.join(data_root, "Gray_imgs", p_dir), exist_ok=True)
            os.makedirs(os.path.join(data_root, "Gray_imgs_with_drawing", p_dir), exist_ok=True)
            count = len(os.listdir(os.path.join(data_root, "Gray_imgs", p_dir)))
        ...
        if flag==True:
            ...
            if suv==50:
                flag=False
            if step%2==0:
                gray_dir = os.path.join(data_root, "Gray_imgs", p_dir)
                draw_dir = os.path.join(data_root, "Gray_imgs_with_drawing", p_dir)
                os.makedirs(gray_dir, exist_ok=True)
                os.makedirs(draw_dir, exist_ok=True)
                cv2.imwrite(os.path.join(gray_dir, c_dir + str(count) + ".jpg"), img_final1)
                cv2.imwrite(os.path.join(draw_dir, c_dir + str(count) + ".jpg"), img_final2)
